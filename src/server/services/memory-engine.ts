...
const MEMORY_EXTRACTOR_PROMPT = `你是一个记忆提取助手。你的任务是从用户与AI角色的对话中，提取关于用户的信息——任何可能对未来互动有价值的内容。

## 核心原则
**When in doubt, EXTRACT.** 宁可多提取也不要遗漏。一条看似普通的偏好，可能在未来的对话中成为关键。

## 提取规则
1. 只提取关于**用户**的信息（第一人称"我"相关），不提取AI角色的信息。
2. 提取**任何可能对未来互动有价值**的信息，包括但不限于：
   - 用户的偏好、习惯、口味（如饮食、音乐、穿搭）
   - 用户的情感状态、恋爱倾向、亲密偏好
   - 用户的色气偏好、敏感话题态度
   - 用户的身份、经历、技能等客观事实
   - 用户提及的重要事件或计划
3. 每条记忆用第三人称表述（如"用户喜欢..."、"用户在..."、"用户偏好..."）。
4. 每条记忆控制在15字以内，极度精炼，只保留核心信息。
5. 如果对话中没有值得记录的新信息，返回空数组。

## 分类定义
- FACT: 用户的客观信息（身份、经历、技能等）。例："用户是程序员"、"用户养了猫"
- PREFERENCE: 用户的主观偏好（喜好、厌恶、习惯、情感倾向等）。例："用户喜欢冰美式"、"用户偏好温柔型"、"用户讨厌被说教"
- EVENT: 用户经历或提及的重要事件。例："用户下周去旅行"、"用户刚换了工作"
...
{
  "memories": [
    { "content": "用户喜欢冰美式", "category": "PREFERENCE" },
    { "content": "用户是程序员", "category": "FACT" }
  ]
}
\`\`\`
...
export class MemoryEngine {
  ...
  async extractAndPersist(
    ...
    try {
      ...
      // 0. 会员频率控制：会员用户每 20 轮对话才触发一次记忆提取
      const user = await userRepository.findById(userId);
      const isMember = user?.vipExpiresAt && new Date(user.vipExpiresAt) > new Date();
      if (isMember) {
        // 查询该 user+character 最近一条记忆的 createdAt
        const [latestMemory] = await db
          .select({ createdAt: memories.createdAt })
          .from(memories)
          .where(and(eq(memories.characterId, characterId), eq(memories.userId, userId)))
          .orderBy(desc(memories.createdAt))
          .limit(1);

        let messageCountSince: number;
        if (latestMemory?.createdAt) {
          // 有历史记忆：计算该记忆创建之后的新消息数
          const [countResult] = await db
            ...
            .where(
              and(
                ...
                gt(messages.createdAt, latestMemory.createdAt),
              ),
            );
          messageCountSince = Number(countResult?.count ?? 0);
        } else {
          // 无历史记忆：计算总消息数
          const [countResult] = await db
            .select({ count: sql<number>`count(*)` })
        ...
        if (messageCountSince < 20) {
          console.log(`[MEMORY] VIP frequency control: only ${messageCountSince} messages since last extraction (threshold: 20), skipping`);
          return;
        }
        console.log(`[MEMORY] VIP frequency control: ${messageCountSince} messages since last extraction, proceeding`);
      }
      ...
      // 7. 容量管理（user 和 isMember 已在步骤 0 获取）
      const maxCapacity = isMember ? 10000 : 100;
  ...
  private _parseResponse(text: string): ExtractionResult | null {
    try {
      ...
      // 容错：memories 为 null 时视为空数组
      if (!parsed) return null;
      const memoriesArray = parsed.memories ?? [];
      if (!Array.isArray(memoriesArray)) return null;
      ...
      for (const item of memoriesArray) {
        if (
          ...
          // 容错：category 大小写兼容（fact→FACT, preference→PREFERENCE, event→EVENT）
          const normalizedCategory = item.category.toUpperCase() as string;
          if (validCategories.has(normalizedCategory)) {
            memories.push({
              content: item.content.slice(0, 15), // 截断过长内容（与Prompt 15字限制对齐）
              category: normalizedCategory as "FACT" | "PREFERENCE" | "EVENT",
            });
          }
        }
      }
